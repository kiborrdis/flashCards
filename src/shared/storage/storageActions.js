...
const createSingleQueryAction = (queryCreator, resultParser = copy) => (
  createAction((executor, ...args) => (
    executor.executeQuery(queryCreator(...args)).then(resultParser)
  ))
);
...
export const getLastTrialForDeck = createSingleQueryAction(
  creators.getLastTrialForDeck, extractSingleResult,
);
...
export const getTrialWithCards = createAction(async (executor, trialId) => {
  const trial = await executor.executeQuery(creators.getTrial(trialId)).then(extractSingleResult);
  const cards = await executor.executeQuery(
    creators.getTrialCardsFromDeck(trial.deckId, trial.numberOfCards),
  );
